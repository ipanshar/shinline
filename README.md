...
- учёт **озеленения** территории (модуль Greenlog);
...
- **Greenlog** — учёт озеленения: растения, локации (карта), задачи по уходу, расходы, отчёты с экспортом в XLSX.
...
│  ├─ Models/                # Eloquent-модели (≈60), вкл. Greenlog/*
...
- **Доменная декомпозиция.** Чёткое разделение на модули (охрана, весовой контроль, гости, спецтехника,
  нарушения, Greenlog) с отдельными сервисами и разрешениями — система масштабируется без «спагетти».
